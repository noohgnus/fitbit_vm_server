...
def token_authorize():
    ...
    # print(r.text)
    file_obj = open("researcher_token.json", 'wb')
...
def get_multi_token_dict():
    print("Reading in from existing researcher_tokens.json ...")
    file_obj = open("researcher_tokens.json", 'r')
    st = json.load(file_obj)
    file_obj.close()
    print("\t...Done reading researcher_tokens.json.")
    return st
...
def update_devices(device_id_pair):
    try:
        ...
        result = cursor.execute("SELECT * FROM Researcher_Devices")
        ...
        for device_id in device_id_pair:
            ...
            result = cursor.execute("INSERT INTO Researcher_Devices \
...
def insert_intraday_dict(time_pair):
    ...
    try:
        ...
        flush_stmt = "DELETE FROM Researcher_Step_HeartRate WHERE fitbit_uid = '%s' AND date(timestamp) = '%s'" % (
            flush_user, flush_date)
        dbhandler.execute(flush_stmt)
        connection.commit()

        # repopulate that day's data
        insert_stmt = "INSERT INTO Researcher_Step_HeartRate (timestamp, fitbit_uid, heart_rate, step_count, distance, activity_level, added_on) VALUES (%s, %s, %s, %s, %s, %s, %s)"
...
def insert_daily_activity(ds):
    ...
    try:
        ...
        flush_date = ds.date
        # flush that day's existing data
        flush_stmt = "DELETE FROM Researcher_Daily_Activities WHERE fitbit_uid = '%s' AND date(timestamp) = '%s'" % (
            flush_user, flush_date)
        dbhandler.execute(flush_stmt)
        connection.commit()
        ...
        insert_stmt = "INSERT INTO Researcher_Daily_Activities (timestamp, fitbit_uid, daily_step_total, " \
...
def get_query_start_date(uid):
    def get_db_last_hr_record():
        ...
        try:
            ...
            stmt = "SELECT * FROM Researcher_Step_HeartRate WHERE fitbit_uid = '%s' ORDER BY timestamp DESC LIMIT 1" % uid
